// submission_stats recompute — shared by BOTH event intakes.
//
// Root-caused bug: the realtime WS intake (collab/src/tracking.js) inserted
// events but never recomputed submission_stats, so the StatsBar read stale
// zeros while the verdict (which reads raw events) saw the real numbers. Both
// intakes now call this after inserting a batch, so stats always reflect the
// events actually stored.
//
// Ported from api/src/routes/events.js (itself a port of events.php).

import {
  countSliceTextLength, extractPlainText, strWordCount, round,
} from './text.js';

export async function recomputeSubmissionStats(pool, sid) {
  ...
  const [jumpRows] = await pool.query(
  ...
  const [pasteCharsRows] = await pool.query(
  ...
  if (lastFocus !== null) {
    const [lr] = await pool.query('SELECT MAX(occurred_at) AS last_ts FROM events WHERE submission_id = ?', [sid]);
  ...
  let wordCount = null;
  const [cRows] = await pool.query('SELECT content FROM submissions WHERE id = ?', [sid]);
  ...
  if (wordCount === null) {
    const [eRows] = await pool.query('SELECT word_count FROM submission_stats WHERE submission_id = ?', [sid]);
