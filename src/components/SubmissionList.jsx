...
export default function SubmissionList({ submissions }) {
  if (!submissions?.length) {
    return <p className="text-sm text-gray-400 py-8 text-center">No submissions yet.</p>;
  }
    ...
    <div className="space-y-2">
      {submissions.map(s => (
        <Link key={s.id} to={`/review/${s.id}`}
          className="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-4 hover:border-primary-300 transition-colors">
          ...
          <div className="flex items-center gap-4">
            {s.avg_wpm > 0 && <span className="text-xs text-gray-500">{s.avg_wpm} WPM</span>}
            {s.total_time_ms > 0 && (
